...
import com.bytewizard.videoactionservice.mapper.CommentMapper;
import com.bytewizard.videoactionservice.service.CommentService;
...
public class CommentServiceImpl extends ServiceImpl<CommentMapper, Comment> implements CommentService {
    ...
    public CommentResponse createCommentVideo(CreateCommentRequest createCommentRequest) {
        ...
        comment.setCommentId(IdUtil.getSnowflake(SnowflakeConstant.WORKER_ID, SnowflakeConstant.DATA_CENTER_ID).nextId());
        ...
        // 更新视频评论数
        if (!videoStatsService.lambdaUpdate().setSql("comment_count = comment_count + 1").eq(VideoStats::getVideoId, createCommentRequest.getVideoId()).update()) {
            throw new BusinessException( ErrorCode.SYSTEM_ERROR, "更新用户评论数数失败");
        }
    ...
    public Boolean deleteCommentVideo(CancelVideoActionRequest cancelVideoActionRequest) {
        ...
        // 删除评论
        if (this.baseMapper.deleteById(cancelVideoActionRequest.getId()) == 0) {
        ...
        // 更新视频评论数
        if (!videoStatsService.lambdaUpdate().set(VideoStats::getCommentCount, countComments).eq(VideoStats::getVideoId, cancelVideoActionRequest.getVideoId()).update()) {
