package com.bytewizard.searchsyncservice.client;
...
import com.bytewizard.searchsyncservice.esdao.VideoEsDao;
...
public class CanalClient implements CommandLineRunner {
    ...
    private void handleMessage(List<CanalEntry.Entry> entries) {
        ...
        for (CanalEntry.Entry entry : entries) {
            ...
            // 跳过 TRANSACTIONBEGIN/END，只处理具体行变更。
            ...
            // 遍历每行变更
            // RowData 包含变更前（before）和变更后（after）的列值。
    ...
    private void insertBulletToRedis(Map<String, String> map) {
        ...
        try {
            // 预防缓存雪崩
            redisTemplate.opsForZSet().add(cacheKey, value, timePoint);
    ...
    private void updateVideoToRedis(Map<String, String> map) {
        try {
            ...
            if (redisTemplate.hasKey(cacheKey)) {
                Map<String, String> videoDetails = redisTemplate.opsForHash().entries(cacheKey).entrySet().stream().collect(Collectors.toMap(e -> e.getKey().toString(), e -> e.getValue().toString(), (a, b) -> b, HashMap::new));
                ...
                redisTemplate.opsForHash().putAll(cacheKey, videoDetails);
                redisTemplate.expire(cacheKey, VideoConstant.VIDEO_DETAIL_DAYS, TimeUnit.DAYS);
            }
